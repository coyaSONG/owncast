...
func TestRun_LegacyDatabaseAtV9(t *testing.T) {
	...
	// The replay migration adds four more tables (streams,
	// video_segment_output_configuration, video_segments, replay_clips).
	var newTableCount int
	mustScan(t, db.QueryRow(`SELECT count(*) FROM sqlite_master WHERE type='table'`), &newTableCount)
	if newTableCount != tableCount+6 { // +1 goose_db_version, +1 federated_servers, +4 replay tables
		t.Errorf("table count changed from %d to %d (expected +6 for goose_db_version + federated_servers + 4 replay tables)", tableCount, newTableCount)
	}
}
